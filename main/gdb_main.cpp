...
int GDB::gdb_main_loop(struct target_controller *tc, bool in_syscall)
{
    ...
    while (1)
    {
        ...
        case '?':
        { /* '?': Request reason for target halt */
        ...
        case 'X':
        { /* 'X addr,len:XX': Write binary data to addr */
            uint32_t addr, len;
            int bin;
            ERROR_IF_NO_TARGET();
        ...
        case 'z':
        { /* z type,addr,len: Clear breakpoint packet */
            ERROR_IF_NO_TARGET();
...
void GDB::handle_q_string_reply(const char *str, const char *param)
{
...
void GDB::handle_q_packet(char *packet, int len)
{
    uint32_t addr, alen;
...
void GDB::handle_v_packet(char *packet, int plen)
{
    unsigned long addr, len;
    int bin;
...
void GDB::handle_z_packet(char *packet, int plen)
{
    (void)plen;
