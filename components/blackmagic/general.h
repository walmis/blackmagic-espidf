...
#define ESP_LOG_LEVEL_BMP(level, tag, format, ...)                                                                     \
	do {                                                                                                           \
		if (level == ESP_LOG_ERROR) {                                                                          \
			esp_log_write(ESP_LOG_ERROR, tag, LOG_FORMAT_BMP(E, format), esp_log_timestamp(), tag,         \
				      ##__VA_ARGS__);                                                                  \
		} else if (level == ESP_LOG_WARN) {                                                                    \
			esp_log_write(ESP_LOG_WARN, tag, LOG_FORMAT_BMP(W, format), esp_log_timestamp(), tag,          \
				      ##__VA_ARGS__);                                                                  \
		} else if (level == ESP_LOG_DEBUG) {                                                                   \
			esp_log_write(ESP_LOG_DEBUG, tag, LOG_FORMAT_BMP(D, format), esp_log_timestamp(), tag,         \
				      ##__VA_ARGS__);                                                                  \
		} else if (level == ESP_LOG_VERBOSE) {                                                                 \
			esp_log_write(ESP_LOG_VERBOSE, tag, LOG_FORMAT_BMP(V, format), esp_log_timestamp(), tag,       \
				      ##__VA_ARGS__);                                                                  \
		} else {                                                                                               \
			esp_log_write(ESP_LOG_INFO, tag, LOG_FORMAT_BMP(I, format), esp_log_timestamp(), tag,          \
				      ##__VA_ARGS__);                                                                  \
		}                                                                                                      \
	} while (0)
...
#define ESP_LOG_LEVEL_BMP(level, tag, format, ...)                                                                     \
	do {                                                                                                           \
		if (level == ESP_LOG_ERROR) {                                                                          \
			esp_log_write(ESP_LOG_ERROR, tag, LOG_SYSTEM_TIME_FORMAT_BMP(E, format),                       \
				      esp_log_system_timestamp(), tag, ##__VA_ARGS__);                                 \
		} else if (level == ESP_LOG_WARN) {                                                                    \
			esp_log_write(ESP_LOG_WARN, tag, LOG_SYSTEM_TIME_FORMAT_BMP(W, format),                        \
				      esp_log_system_timestamp(), tag, ##__VA_ARGS__);                                 \
		} else if (level == ESP_LOG_DEBUG) {                                                                   \
			esp_log_write(ESP_LOG_DEBUG, tag, LOG_SYSTEM_TIME_FORMAT_BMP(D, format),                       \
				      esp_log_system_timestamp(), tag, ##__VA_ARGS__);                                 \
		} else if (level == ESP_LOG_VERBOSE) {                                                                 \
			esp_log_write(ESP_LOG_VERBOSE, tag, LOG_SYSTEM_TIME_FORMAT_BMP(V, format),                     \
				      esp_log_system_timestamp(), tag, ##__VA_ARGS__);                                 \
		} else {                                                                                               \
			esp_log_write(ESP_LOG_INFO, tag, LOG_SYSTEM_TIME_FORMAT_BMP(I, format),                        \
				      esp_log_system_timestamp(), tag, ##__VA_ARGS__);                                 \
		}                                                                                                      \
	} while (0)
