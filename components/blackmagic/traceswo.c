...
static uint32_t uart_baud_detect(uart_port_t uart_num, int sample_bits)
{
	int low_period = 0;
	int high_period = 0;
...
static void swo_uart_rx_task(void *arg)
{
	...
	uint32_t baud_rate = default_baud;
	esp_err_t ret;
	uint8_t buf[256];
	int bufpos = 0;
	...
	ret = uart_set_pin(CONFIG_TRACE_SWO_UART_IDX, UART_PIN_NO_CHANGE, CONFIG_TDO_GPIO, UART_PIN_NO_CHANGE, UART_PIN_NO_CHANGE);
	if (ret != ESP_OK) {
		ESP_LOGE(TAG, "unable to configure RMT pin: %s", esp_err_to_name(ret));
		goto out;
	}
	...
	if (ret != ESP_OK) {
		ESP_LOGE(TAG, "unable to install RMT uart driver: %s", esp_err_to_name(ret));
		goto out;
	}

	ret = uart_param_config(CONFIG_TRACE_SWO_UART_IDX, &uart_config);
	if (ret != ESP_OK) {
		ESP_LOGE(TAG, "unable to configure RMT uart driver: %s", esp_err_to_name(ret));
		goto out;
	}

	if (baud_rate == 0) {
		ESP_LOGD(TAG, "baud rate not specified, initiating autobaud detection");
		baud_rate = uart_baud_detect(CONFIG_TRACE_SWO_UART_IDX, 20);
	}
	...
	if (ret != ESP_OK) {
		ESP_LOGE(TAG, "unable to configure RMT interrupt: %s", esp_err_to_name(ret));
		goto out;
	}

	ESP_LOGI(TAG, "RMT driver started with baud rate of %d, beginning reception...", baud_rate);
	...
	while (1) {
		...
		if (xQueueReceive(uart_event_queue, (void *)&evt, 100)) {
			...
			if (evt.type == 0x1000) {
				...
				if (evt.size == 0) {
					ESP_LOGD(TAG, "retecting baud rate");
					baud_rate = uart_baud_detect(CONFIG_TRACE_SWO_UART_IDX, 20);
